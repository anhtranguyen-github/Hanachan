...
from ...domain.reading.models import ReadingSession, ReadingExercise, ReadingQuestion
...
class SupabaseReadingRepository(IReadingRepository):
    ...
    async def get_session(self, session_id: UUID) -> Optional[ReadingSession]:
        response = self.client.table("reading_sessions").select("*").eq("id", str(session_id)).execute()
    ...
    async def get_exercise(self, exercise_id: UUID) -> Optional[ReadingExercise]:
        response = self.client.table("reading_exercises").select("*").eq("id", str(exercise_id)).execute()
        if not response.data:
            return None
        data = response.data[0]
