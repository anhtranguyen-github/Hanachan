...
from app.domain.learning.models import Rating, SRSState, SRSStage
...
async def start_review(
    ...
    query = client.table("user_fsrs_states") \
...
async def submit_review(
    ...
    # Get current state
    state_res = client.table("user_fsrs_states") \
        .select("*") \
        .eq("user_id", user_id) \
    ...
    next_review, next_state = FSRSEngine.calculate_next_review(current_state, rating_enum, payload.wrong_count)
    ...
    }, on_conflict="user_id,item_id,item_type,facet").execute()
    ...
    }, on_conflict="session_id,ku_id,facet").execute()
...
async def complete_lesson(
    ...
    try:
        ...
        client.table("lesson_items").update({
            "status": "learned"
