...
from .models import SRSState, SRSStage, Rating
...
class FSRSEngine:
    ...
    def calculate_next_review(
        ...
        # CASE C: Pure Success
            ...
            if reps == 1 and stability < 0.166: stability = 0.166 # 4h
            elif reps == 2 and stability < 0.333: stability = 0.333 # 8h
            elif reps == 3 and stability < 1.0: stability = 1.0 # 1d
            elif reps == 4 and stability < 3.0: stability = 3.0 # 3d
            else:
...
class LearningService:
    ...
    async def get_ku_progress(self, user_id: str, identifier: str, include_notes: bool = False) -> Optional[KUStatus]:
        """
        Retrieves learning progress for a KU by ID, character, or slug.
        """
        # 1. Resolve identifier to KU ID if it's not a UUID
        # Simple heuristic: if length is 1 or it's non-UUID like, search it
        ku = None
    ...
    async def submit_review(self, user_id: str, ku_id: str, facet: str, rating: Rating, wrong_count: int = 0):
        # 1. Get current state
