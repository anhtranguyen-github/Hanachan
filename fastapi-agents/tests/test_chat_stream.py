...
async def test_chat_stream(authenticated_client: AsyncClient, sample_user_id: str):
    ...
    async def mock_astream_events(*args, **kwargs):
        ...
        yield {"event": "on_chain_end", "name": "planner", "data": {"output": {"thought": "I decided to use these tools: ['search_knowledge']"}}}
        
        # 2. Tool Trace
        yield {"event": "on_tool_start", "name": "search_knowledge", "data": {"input": {"query": "test"}}}
        ...
        yield {"event": "on_chat_model_stream", "data": {"chunk": AIMessageChunk(content="streamed response")}}

        # 4. Final Thought
        yield {"event": "on_chain_end", "name": "generate", "data": {"output": {"thought": "Final answer generated."}}}

    with patch("app.api.v1.endpoints.chat.memory_graph.astream_events", side_effect=mock_astream_events):
        ...
        async with authenticated_client.stream(
            ...
            tokens = [json.loads(line)["content"] for line in data_lines if '"type": "token"' in line]
