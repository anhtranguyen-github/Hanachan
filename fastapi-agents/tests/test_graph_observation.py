...
async def test_graph_thought_tracing_observation():
    ...
    # Mock Messages
    mock_planner_1 = AIMessage(
        content="",
        tool_calls=[{"name": "search_knowledge_units", "args": {"query": "桜"}, "id": "call_1"}],
    )
    ...
    with (
        ...
        with (
            patch("app.agents.memory_agent.TOOLS", [mock_tool]),
            ...
            assert (
                "sufficient" in thoughts[2].lower() or "answer" in thoughts[2].lower()
            )  # Index 2 is generator or second planner
...
async def test_graph_loop_observation():
    ...
    rs = [
        AIMessage(
            content="",
            tool_calls=[{"name": "search_knowledge_units", "args": {"q": "x"}, "id": "c1"}],
        ),
    ...
    with (
        ...
        with patch("app.agents.memory_agent.TOOLS", [mock_tool]):
            ...
            rewrite_thoughts = [
                e["data"]["output"]["thought"]
                for e in events
                if e["event"] == "on_chain_end"
                and e["name"] == "rewrite"
                and "thought" in e["data"].get("output", {})
            ]
