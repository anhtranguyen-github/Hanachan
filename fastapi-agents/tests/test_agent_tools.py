...
async def test_tool_get_user_learning_progress():
    ...
    with patch("app.services.mcp_domain_client.MCPDomainClient.call_tool", new_callable=AsyncMock) as mock_call:
        ...
        mock_call.assert_awaited_once_with("get_learning_progress", {"user_id": "u1", "identifier": "桜"})
...
async def test_tool_search_knowledge_units():
    ...
    with patch("app.services.mcp_domain_client.MCPDomainClient.call_tool", new_callable=AsyncMock) as mock_call:
        ...
        result = await search_knowledge_units.ainvoke({"jwt": "jwt", "query": "sakura", "user_id": "u1"})
...
async def test_tool_create_user_deck():
    ...
    with patch("app.services.mcp_domain_client.MCPDomainClient.call_tool", new_callable=AsyncMock) as mock_call:
        ...
        result = await create_user_deck.ainvoke({
            "name": "JLPT N2", 
            "jwt": "token123", 
            "description": "N2 Vocab", 
            "user_id": "user1"
        })
        
        assert "Successfully created deck" in result
        mock_call.assert_awaited_once_with("create_deck", {
            "user_id": "user1", 
            "name": "JLPT N2", 
            "description": "N2 Vocab"
        })
