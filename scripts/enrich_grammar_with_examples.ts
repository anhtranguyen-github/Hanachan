...
async function main() {
    ...
    console.log(`Starting to seed examples for ${allItems.length} grammar points...`);
    ...
    for (let i = 0; i < allItems.length; i++) {
        ...
        if (i % 20 === 0) console.log(`Progress: ${i}/${allItems.length} (${item.slug})`);

        if (item.examples && item.examples.length > 0) {
            for (const ex of item.examples) {
                // 1. Insert/Get Sentence
                // We use text_ja + text_en for conflict check if we had a unique constraint, 
                // but we'll just insert and get the ID.
                const { data: sentence, error: sErr } = await supabase.from('sentences').upsert({
                ...
                }, { onConflict: 'text_ja,origin' }).select('id').single();

                if (sErr || !sentence) {
                    // console.error(`Error sentence for ${item.slug}:`, sErr);
                    continue;
                }

                // 2. Link in grammar_sentences
                await supabase.from('grammar_sentences').upsert({
                    grammar_id: kuId,
                    sentence_id: sentence.id
                }, { onConflict: 'grammar_id,sentence_id' });

                // 3. Link in ku_to_sentence
        ...
        // Update content_blob with more details if needed
