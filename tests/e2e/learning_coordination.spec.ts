...
test.describe('E2E Learning Coordination Flow', () => {
    ...
    test('Full Integration: Learn -> Analytics -> Dashboard -> Review', async ({ page }) => {
        ...
        if (parseInt(initialLessons) > 0) {
            ...
            const allClear = page.getByText(/completed all current items/i);
            await Promise.race([
                startLink.waitFor({ state: 'visible' }),
                allClear.waitFor({ state: 'visible' })
            ]).catch(() => { });
                ...
                if (page.url().includes('/learn/session')) {
                    ...
                    while (!isQuiz && slideCount < 10) {
                        ...
                        if (await nextBtn.isVisible()) {
                            ...
                            if (text.includes('Quiz')) {
                                ...
                                await page.getByTestId('debug-answer').waitFor({ state: 'attached' }).catch(() => { });
                            } else {
                                // Wait for the next slide title or button to re-appear to ensure transition
                                await page.waitForResponse(resp => resp.url().includes('/learn/session')).catch(() => { });
                            }
                        } else {
                    ...
                    while (!isComplete && loopCount < 40) {
                        ...
                        if (await debugEl.count() > 0) {
                            ...
                            await page.waitForResponse(resp => resp.status() === 200).catch(() => { });
                    ...
                    await expect(backToDash).toBeVisible();
                    ...
                    const postLearnLessons = await page.getByTestId('learn-new-count').innerText();
                    ...
                    if (supabaseKey) {
                        ...
                        const dueCount = await page.getByTestId('review-due-count').innerText();
                    ...
                    while (!reviewComplete && loopCount < 20) {
                        ...
                        if (await debugEl.count() > 0) {
                            ...
                            if (await page.getByText('Review Pipeline').isVisible() && await page.getByText('Active Queue').isVisible()) {
                                reviewComplete = true;
                            } else {
                    ...
                    const finalDue = await page.getByTestId('review-due-count').innerText();
