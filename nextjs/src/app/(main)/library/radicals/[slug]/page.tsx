...
export default async function RadicalDetailPage({ params }: { params: { slug: string } }) {
    ...
    if (!radical) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-8">
            <div className="bg-white border border-border rounded-3xl p-8 text-center max-w-sm w-full shadow-sm">
                ...
                <Link href="/library?type=radical" className="block w-full py-3 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-widest text-center hover:opacity-90 transition-opacity">
                    Back to Radicals
                </Link>
            </div>
        </div>
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-700 pb-8">
            {/* Breadcrumb */}
            <Link href="/library?type=radical" className="inline-flex items-center gap-2 text-foreground/40 hover:text-foreground transition-colors group text-sm">
