...
export default function DashboardPage() {
    ...
    return (
        <div data-testid="dashboard-root" className="max-w-5xl mx-auto space-y-sm font-sans text-foreground animate-in fade-in duration-1000">
            {/* Core Action Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Link
                    href="/review"
                    data-testid="review-card"
                    className="bg-white border-2 border-border/60 rounded-[32px] p-8 flex flex-col items-center justify-center text-center space-y-3 shadow-sm hover:border-primary transition-all group min-h-[240px]"
                >
                    <div className="w-12 h-12 rounded-full bg-[#FFF5F5] flex items-center justify-center text-primary shadow-inner">
                        <TrendingUp size={24} />
                    </div>
                    <div className="space-y-0.5">
                        <h3 className="text-lg font-black uppercase tracking-tight text-[#3E4A61]">
                            <span data-testid="review-due-count">{stats.due}</span> Reviews Due
                        </h3>
                        <p className="text-[10px] text-[#A0AEC0] font-bold italic tracking-tight group-hover:text-primary transition-colors">Time to practice your knowledge!</p>
                    </div>
                </Link>

                <div className="bg-white border-2 border-border/60 rounded-[32px] p-7 shadow-xl shadow-[#3E4A61]/5 flex flex-col justify-between relative overflow-hidden group min-h-[240px]">
                    ...
                    <div className="relative z-10 space-y-3">
                        <div className="space-y-0.5">
                            <div className="flex items-center gap-2">
                                <BookOpen size={12} className="text-[#3E4A61]" />
                                <span className="text-[8px] font-black uppercase tracking-widest text-[#3E4A61]">New Content</span>
                            </div>
                            <h3 className="text-2xl font-black text-[#3E4A61]">Level {userLevel}</h3>
                            <p className="text-[10px] font-black text-primary tracking-tight">Next Discovery Batch</p>
                        </div>

                        <div className="bg-[#F2E8E8] rounded-xl p-3 flex items-center justify-between">
                            <span className="text-[9px] font-black uppercase tracking-widest text-[#3E4A61]">
                                <span data-testid="learn-new-count">{stats.new}</span> New Lessons
                            </span>
                            <div className="flex -space-x-2">
                                {[1, 2, 3].map(i => (
                                    <div key={i} className="w-5 h-5 rounded-full bg-white border border-[#F2E8E8] flex items-center justify-center text-[7px] text-[#A0AEC0]">?</div>
                                ))}
                                <div className="w-5 h-5 rounded-full bg-primary border border-primary flex items-center justify-center text-[7px] text-white font-bold">+</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <Link
                        ...
                        className="relative z-10 mt-3 w-full py-3 px-6 border border-border rounded-xl flex items-center justify-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-[#3E4A61] hover:bg-[#3E4A61] hover:text-white hover:border-[#3E4A61] transition-all group/btn shadow-sm"
                    >
                        Unlock Batch
