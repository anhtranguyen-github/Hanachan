...
export async function GET(req: NextRequest) {
  ...
  try {
    const url = `${AGENTS_BASE_URL}/api/v1/memory/sessions`;
    console.log('[BFF] Fetching from upstream:', url);
    ...
    if (!upstream.ok) {
      console.error('[BFF] Sessions Upstream Error:', upstream.status, text);
    }
    ...
    console.error('[BFF] Sessions Proxy Exception:', err);
