...
export async function GET(_req: NextRequest, ctx: { params: { session_id: string } }) {
  ...
  try {
    const upstream = await fetch(`${AGENTS_BASE_URL}/api/v1/memory/session/${encodeURIComponent(session_id)}`, {
...
export async function DELETE(_req: NextRequest, ctx: { params: { session_id: string } }) {
  ...
  try {
    const before = await fetch(`${AGENTS_BASE_URL}/api/v1/memory/session/${encodeURIComponent(session_id)}`, {
      headers: { Authorization: bearer },
      cache: 'no-store',
    });
