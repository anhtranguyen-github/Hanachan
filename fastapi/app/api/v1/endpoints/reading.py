...
async def update_reading_config(
    ...
    if existing:
        ...
        execute_query(
            f"UPDATE public.reading_configs SET {set_clauses}, updated_at = NOW() WHERE user_id = %s",  # noqa: S608 # nosec B608
            values,
            fetch=False,
        )
    else:
        ...
        execute_query(
            f"INSERT INTO public.reading_configs ({cols}) VALUES ({placeholders})",  # noqa: S608 # nosec B608
...
async def list_reading_sessions(
    ...
    total = execute_query(
        f"SELECT COUNT(*) as count FROM public.reading_sessions WHERE {where_sql}",  # noqa: S608 # nosec B608
        count_params,
    )
