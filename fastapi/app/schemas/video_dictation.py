...
class VideoDictationSettings(BaseModel):
    ...
    min_subtitle_length: int = Field(1, ge=1, description="Minimum subtitle length in characters")
    max_subtitle_length: int = Field(100, le=1000, description="Maximum subtitle length in characters")
    enable_reading_hint: bool = Field(False, description="Show hiragana reading hint")
    playback_speed: float = Field(1.0, ge=0.5, le=2.0, description="Audio playback speed")
...
class CreateDictationSessionRequest(BaseModel):
    ...
    video_id: str = Field(..., min_length=1, max_length=100, description="Video ID to practice dictation with")
...
class SubmitDictationAttemptRequest(BaseModel):
    ...
    user_input: str = Field(..., min_length=1, max_length=1000, description="What the user typed")
    time_taken_ms: int = Field(0, ge=0, description="Time taken to type")
...
class DictationAttemptResult(BaseModel):
    ...
    accuracy_score: int = Field(..., ge=0, le=100, description="Similarity score 0-100")
    correct_chars: int = Field(..., ge=0, description="Number of correct characters")
    total_chars: int = Field(..., ge=0, description="Total characters in target")
