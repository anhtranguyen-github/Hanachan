...
class PracticeSentenceSchema(BaseModel):
    ...
    learned_words_count: int = Field(
        ..., description="Number of learned words in sentence"
    )
    audio_url: Optional[str] = Field(
        None, description="Audio URL for native pronunciation"
    )
...
class AdaptiveFeedback(BaseModel):
    ...
    next_action: str = Field(
        ..., description="What to do next: repeat, simpler, next, advance, mastered"
    )
    next_difficulty: str = Field(
        ..., description="Recommended difficulty for next sentence"
    )
    reason: str = Field(..., description="Human-readable reason for the recommendation")
    should_repeat: bool = Field(
        ..., description="Whether to repeat the current sentence"
    )
...
class PracticeSessionResponse(BaseModel):
    ...
    sentences: List[PracticeSentenceSchema] = Field(
        default_factory=list, description="Selected sentences for practice"
    )
    ...
    total_sentences: int = Field(
        ..., description="Total number of sentences in session"
    )
...
class NextPracticeItemResponse(BaseModel):
    ...
    sentence: Optional[PracticeSentenceSchema] = Field(
        None, description="Next sentence to practice"
    )
    ...
    feedback: Optional[AdaptiveFeedback] = Field(
        None, description="Adaptive feedback based on last attempt"
    )
