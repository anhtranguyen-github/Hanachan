...
def search_knowledge_units(query: str) -> str:
    ...
    return "\n".join([f"- {r['character']} ({r['meaning']}) [slug: {r['slug']}, type: {r['type']}]" for r in results])
...
def tools_node(state: AgentState) -> Dict[str, Any]:
    ...
    for tool_call in last_msg.tool_calls:
        ...
        if tool_name in ["get_episodic_memory", "get_semantic_facts", "get_user_learning_progress"]:
...
def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    thread_text = sess_mem.get_thread_context_text(state["session_id"], last_n=5) if state.get("session_id") else "No active session."
...
def rewriter_node(state: AgentState) -> Dict[str, Any]:
    ...
    return {
        "messages": [HumanMessage(content=f"[Reviewer Feedback]: The previous tools didn't find enough. {suggestion}")]
    }
...
def update_memory_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Summary
        summary = summary_llm.invoke(f"Summarize this interaction in one sentence: User: {user_input}\nAI: {output}").content
...
def _build_graph():
    ...
    workflow.add_conditional_edges("reviewer", decide_path, {
        "rewrite": "rewrite",
        "generate": "generate"
    })
