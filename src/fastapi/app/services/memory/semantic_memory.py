...
def _get_driver() -> Driver:
    global _driver
    if _driver is None:
        uri = settings.neo4j_uri if settings.neo4j_uri else "bolt://localhost:7687"
        user = settings.neo4j_user if settings.neo4j_user else "neo4j"
        password = settings.neo4j_password if settings.neo4j_password else "password"
